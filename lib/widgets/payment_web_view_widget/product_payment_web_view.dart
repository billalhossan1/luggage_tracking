...
void productWebView(BuildContext context, String paymentUrl) {
  final logger = Logger();
    ...
    ..setNavigationDelegate(
      NavigationDelegate(
        onNavigationRequest: (NavigationRequest request) {
          // Handle success URL
          if (request.url.startsWith(Urls.productSuccessUrl)) {
          ...
          // Handle cancel URL
          if (request.url.startsWith(Urls.productCancelUrl)) {
            logger.i("Payment Cancelled URL detected");
            showDialog(
              context: context,
              builder: (context) => AlertDialog(
                title: const Text('Payment Failed'),
                content: const Text('Your payment was Failed.'),
                actions: [
                  TextButton(
                    onPressed: () {
                      Get.offAllNamed(AppRoutes.instance.navigationScreen);
