...
class NotificationService {
  ...
  Future<void> setupFCM() async {
    try {
      ...
      if (token != null) {
        ...
        await SaveDataController().saveFcmToken(token);
        String? savedToken =
            await SaveDataController().getFcmToken();
  ...
  void _handleForegroundMessage(RemoteMessage message) {
    ...
    Get.snackbar(message.notification?.title ?? 'Notification',
        ...
        backgroundColor: AppColors.instance.black900, colorText: Colors.white, onTap: (x) {
      Get.toNamed(
